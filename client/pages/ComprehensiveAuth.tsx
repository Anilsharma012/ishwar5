...
const ComprehensiveAuth = () => {
  ...
  const handlePasswordAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // ✅ FIX: use the full response (success/data/error)
      const res: AnyObj = await api.post(endpoint, payload);
      ...
      if (!token || !user) throw new Error("Malformed response from server");
  ...
  const handleOTPSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // ✅ FIX: use full response
      const res: AnyObj = await api.post("auth/firebase-login", {
        idToken,
  ...
  const handleGoogleAuth = async (e?: React.MouseEvent) => {
    ...
    try {
      ...
      // ✅ FIX: use full response (res.success/res.data)
      const res: AnyObj = await api.post("auth/google", {
        idToken,
