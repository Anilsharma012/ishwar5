...
export const getAdminNotificationCounts: RequestHandler = async (req, res) => {
  try {
    ...
    // 1) Properties awaiting approval (includes paid ones waiting for manual approval)
    const pendingCount = await db
      .collection("properties")
      .countDocuments({ approvalStatus: { $in: ["pending", "pending_approval"] } });

    // 2) Re-submitted (still pending but updated after creation)
    const resubmittedCount = await db.collection("properties").countDocuments({
      approvalStatus: { $in: ["pending", "pending_approval"] },
      $expr: { $gt: ["$updatedAt", "$createdAt"] },
    });

    // 3) Reports pending review (best-effort)
    ...
    // 4) Bank transfers pending manual verification (best-effort)
    const bankTransfersPending =
      (await db
        .collection("bank_transfers")
        .countDocuments({ status: "pending" })
        .catch(() => 0)) || 0;

    // 5) Testimonials/Reviews pending moderation (best-effort)
    ...
    // 6) Web enquiries / contact queries pending (status 'new' or 'pending' or 'open')
    //    Prefer collection 'enquiries'; fallback to 'web_queries' if that's what you used.
    const enquiryFilter = { status: { $in: ["new", "pending", "open"] } };

    let enquiriesPending =
      (await db.collection("enquiries").countDocuments(enquiryFilter).catch(() => 0)) || 0;

    if (enquiriesPending === 0) {
      // fallback if your collection is named differently
      enquiriesPending =
        (await db.collection("web_queries").countDocuments(enquiryFilter).catch(() => 0)) || 0;
    }
